...
@interface LLCustomPageView()<iCarouselDataSource,iCarouselDelegate>
@property (nonatomic,strong) UIImageView *firstImageView;
@property (nonatomic,strong) iCarousel *carouselView;
@property (nonatomic,strong) NSMutableArray *dataSource;
@end
@implementation LLCustomPageView
- (UIImageView *) firstImageView{
    if (!_firstImageView) {
        _firstImageView = [UIImageView new];
        _firstImageView.frame = CGRectMake(0, 0, self.bounds.size.width * 0.5, self.bounds.size.height);
...
+ (LLCustomPageView *)pageViewWithFrame:(CGRect)frame{
    LLCustomPageView *instance = [[self alloc] initWithFrame:frame];
    return instance;
}
- (void)layoutSubviews {
